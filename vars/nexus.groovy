...
def code_build(APP_TYPE,COMPONENT) {
    if(APP_TYPE == 'NODEJS') {
        ...
        command = " sudo apt update && wget -c https://dl.google.com/go/go1.14.2.linux-amd64.tar.gz -O - | sudo tar -xz -C /usr/local && export PATH=$PATH:/usr/local/go/bin  && go version  && git clonehttps://github.com/zelar-soft-todoapp/login.git  && apt install go-dep && go get && go build"
        def execute_com = sh(returnStdout: true, script: command)
        print execute_com
    }
    else if(APP_TYPE == 'JAVA') {
        command = "sudo apt update && apt install maven -y && useradd -m -s /bin/bash todo && cd /home/todo && git clone https://github.com/zelar-soft-todoapp/todo.git  && unzip /tmp/users.zip && mv users-main users && cd users && mvn clean package && mv target/user-api-0.0.1.jar user.jar"
        def execute_com = sh(returnStdout: true, script: command)
        print execute_com
    }
