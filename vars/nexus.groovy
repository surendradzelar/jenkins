...
def code_build(APP_TYPE,COMPONENT) {
    if(APP_TYPE == 'NODEJS') {
        ...
        command = "sudo apt-get update && sudo apt-get install maven -y  && git clone https://github.com/zelar-soft-todoapp/todo.git  && mv users-main users && cd users && mvn clean package && mv target/user-api-0.0.1.jar user.jar"
        def execute_com = sh(returnStdout: true, script: command)
        print execute_com
    }
