...
def code_build(APP_TYPE,COMPONENT) {
    if(APP_TYPE == 'NODEJS') {
        ...
        command = " sudo apt-get update && wget -c https://dl.google.com/go/go1.14.2.linux-amd64.tar.gz -O - | sudo tar -xz -C /usr/local && export PATH=$PATH:/usr/local/go/bin  && go version   && sudo apt-get install go-dep && go get && go build"
        def execute_com = sh(returnStdout: true, script: command)
        print execute_com
    }
    else if(APP_TYPE == 'JAVA') {
